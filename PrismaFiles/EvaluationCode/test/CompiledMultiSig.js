let names = {
  testName: "CompiledMultiSig",
  testedContract : "CompiledMultiSig",
...
let submitMultiSigTx = async function(instance, target, functionName, data1, data2, threshold, value, accounts, logName){
  ...
  let fill = function(address){
    if(address.length < 34){
      return "0x" + address.substring(2).padStart(32, "0")
    } else {
      return address
    }
  }

  let nonceE = web3.eth.abi.encodeParameters(["uint"],[nonce.toString(10)]);
  let timeoutE = web3.eth.abi.encodeParameters(["uint"],[timeout]);
  let valueE = web3.eth.abi.encodeParameters(["uint"],[value]);

  let encodePacked = fill(instance.address) + nonceE.substring(2) + timeoutE.substring(2) + fill(target.address).substring(2) + valueE.substring(2) + txData.substring(2)
  ...
  let tx = await instance[names.executeMethod]({signatures:signatures, indices:indices, timeout:timeout, destination:target.address, value:value, data:txData});
  gasLog.push({
      function: logName,
