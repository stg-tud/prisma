let names = {
  testName: "CompiledRPS",
  testedContract : "CompiledRPS",
  commit: "commit",
  move : "move",
  open : "open",
  timeout: "open"
...
let oneRun = async function(_move1, _move2, _nonce, _expection, accounts, timeout = false){
  ...
  if(timeout){
    ...
    tx = await instance[names.timeout]({rps1: 0, nonce: 0}, {from: accounts[1]});
    gasLog.push({
        function: "timeout",
        gas: tx.receipt.gasUsed
    });
    ...
    tx = await instance[names.open]({rps1: _move1, nonce: _nonce});
    gasLog.push({
        function: "open",
        gas: tx.receipt.gasUsed
    });
  ...
  assert.equal(_expection, await instance.winner.call());
